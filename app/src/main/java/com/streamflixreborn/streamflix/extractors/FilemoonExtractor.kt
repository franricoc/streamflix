...
open class FilemoonExtractor : Extractor() {
    ...
    override suspend fun extract(link: String): Video {
        ...
        val currentDomain =
            Regex("""(https?://[^/]+)""").find(link)?.groupValues?.get(1)
        ...
        val playbackDomain =
            Regex("""(https?://[^/]+)""").find(embedFrameUrl)?.groupValues?.get(1)
        ...
        val attestPayload: Map<String, kotlin.Any> =
            mapOf(
        ...
        val playbackPayload: Map<String, kotlin.Any> =
            mapOf(
                "fingerprint" to
                    mapOf(
                        "token" to token,
        ...
        val sources =
            jsonObject.optJSONArray("sources")
    ...
    private fun ByteArray.stripLeadingZero(): ByteArray =
        if (this.isNotEmpty() && this[0] == 0.toByte()) {
    ...
    data class Attestation(
        ...
        val publicKey: Map<String, kotlin.Any>,
    )
    ...
    private interface Service {
        ...
        suspend fun attest(
            @Url url: String,
            @Body body: Map<String, kotlin.Any>,
            @HeaderMap headers: Map<String, String>,
        ): AttestResponse
            ...
            fun build(baseUrl: String): Service {
                val cookieJar =
                    object : okhttp3.CookieJar {
                        ...
                        override fun loadForRequest(url: okhttp3.HttpUrl): List<okhttp3.Cookie> = cookieStore[url.host] ?: ArrayList()
                    }
